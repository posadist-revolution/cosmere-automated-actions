...
export async function breatheStormlight(item: CosmereItem, actor: CosmereActor){
  ...
  log(`Current investiture is ${actor.system.resources.inv.value} out of ${actor.system.resources.inv.max.value}`);
  let invToRefill = 0;
...
async function drainRandomSphere(actSpheres: CosmereItem[], actor: CosmereActor){
  ...
  log(`Draining from sphere #${sphereNumberToDrainFrom}`);
  totalNumSpheres = 0;
